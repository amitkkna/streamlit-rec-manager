...
def create_receivables_report(df_invoices, df_payments, from_date, to_date, group_by):
    """
    Your existing logic for Receivables with partial payments...
    """
    ...
    if group_col:
        df_machine = filtered_df.groupby(group_col)["Machine Revenue"].sum().rename("Machine OS")
        df_parts   = filtered_df.groupby(group_col)["Parts Revenue"].sum().rename("Parts OS")
        df_service = filtered_df.groupby(group_col)["Service Revenue"].sum().rename("Service OS")
    else:
        df_machine = pd.Series(
            filtered_df["Machine Revenue"].sum(),
            index=["Grand Total"],
            name="Machine OS"
        )
        df_parts = pd.Series(
            filtered_df["Parts Revenue"].sum(),
            index=["Grand Total"],
            name="Parts OS"
        )
        df_service = pd.Series(
            filtered_df["Service Revenue"].sum(),
            index=["Grand Total"],
            name="Service OS"
        )
    ...
    if group_col is None:
        ...
        final_df.rename(columns={group_by: "Group"}, inplace=True)
...
# --------------------------------------------------------------------------------
# SEGMENT WISE FUNCTION (SAME LOGIC)
# --------------------------------------------------------------------------------
def create_segment_wise_report(df_invoices, df_payments, company="All Companies"):
    """
    Your partial payment logic with multi-bucket approach...
    """
...
def show_segment_wise():
    ...
    if st.button("Generate Segment-Wise Report"):
        ...
        def highlight_balance(row):
            ...
            row.name is e.g. ("Machine","Balance OS")
            """
            if row.name[1] == "Balance OS":
                # You could choose a color like pale yellow (#FFFACD) or #FFEB3B
...
group_opts = ["Grand Total", "Customer ID", "Company Name", "Customer Name"]
